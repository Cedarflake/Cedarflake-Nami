...
**这个项目面向谁。** 它面向个人使用和工程实验，当前也作为 Revaea 的边缘重定向基础设施。它不是托管短链接服务，也不以企业级重定向平台为目标。目前也没有把内部插件或 SDK 发布为公开包的计划；扩展仍以仓库内的源码级、编译期模块为主。

**这个文档站面向谁。** 它记录维护项目所需的架构、部署与运维方法；其他对实现感兴趣的开发者，也可以按照文档研究、自行部署或扩展自己的实例。

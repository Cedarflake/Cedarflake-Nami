...
the `@nami/database-d1/rest` transport with the account ID, database ID, and a
...
migration, and test infrastructure lives in `@nami/database-d1`.

## Checks

```bash
pnpm --filter @nami/plugin-data-repository-d1 check
pnpm --filter @nami/plugin-data-repository-d1 test
```

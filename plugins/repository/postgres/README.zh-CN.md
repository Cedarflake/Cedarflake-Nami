...
迁移与 seed 命令都会修改所配置的数据库，不得把它们当成验证命令运行。正常首次部署应先执行迁移，再由 WebUI 在 GitHub 身份认证和共享实例密钥校验后原子创建两份文档。seed 继续用于受控的非交互迁移；它会先校验两份文件，再在同一个事务中仅创建缺失文档，不会覆盖已有内容。
...
- 每次初始化、保存、导入、迁移和恢复都会写入不可变历史；恢复旧内容会创建新的活动 revision。
- 本包使用独立迁移表，不与统计迁移混用。

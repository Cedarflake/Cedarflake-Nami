...
路由规则存放在同一分支的 `redirects.json`，其结构由 [redirects.schema.json](redirects.schema.json) 描述，其中也包含单上游高级反向代理的可选 `proxyOptions` 契约。两份 data 文档的 Schema 统一归入此包，可避免 WebUI 编辑器反向依赖 Runtime 应用包。
